...
- Используем ML-классификатор (лёгкая модель на базе BERT), который разделит сообщения по определенным классам в зависмости от уверенности.
...
- На основе этих метрик можно сформировать матрицу взаимодействий где строками будут пользователи, а каналы - столбцами. Для каждой ячейки такой матрицы будут вычисляться interaction*score's.
  ...
  - Фильтрация мусора:
    Исключаем каналы/стримы, которые неактивны (из channel_statistics). Исключаем каналы/стримы, которые неактивны.
