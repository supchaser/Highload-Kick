...
<h3 id="301">Продуктовые метрики</h3>
...
| Действие пользователя              | Среднее кол-во в день для одного юзера | RPS  |
| ---------------------------------- | -------------------------------------- | ---- |
| Регистрация и Авторизация          | 0.1                                    | 4.2  |
| Создание трансляции                | 0.025                                  | 1    |
| Отправка сообщения в чат           | 1.5                                    | 62.5 |
| Обновление страницы (рекомендации) | 2                                      | 83.3 |
| Просмотр своего канала             | 0.1                                    | 4.2  |
| Редактирование своего канала       | 0.01                                   | 0.42 |
| Просмотр онлайн трансляции         | 2.4                                    | 100  |
| Отслеживание канала                | 0.2                                    | 8.33 |
| Оплата подписки                    | 0.02                                   | 0.83 |
| Удаление сообщений                 | 0.05                                   | 2.08 |
| Блокировка других пользователей    | 0.01                                   | 0.42 |
| Создание клипа                     | 0.12                                   | 5.0  |
| Сохранение трансляции              | 0.03                                   | 1.25 |
| Просмотр видеоплеера               | 1                                      | 41.7 |
| Пожаловаться на трансляцию         | 0.0005                                 | 0.02 |

Расчёт RPS был проведён по формуле: PRS = (Среднее кол-во в день для одного юзера) \* DAU / 86400.
Суммарный средний RPS: 315.25. Тогда пиковый RPS - 315.25 \* 2 =630.5.
...
- Размер хранения в разбивке по типам данных (в штуках и в Тб) - для существенных блоков данных: [таблица](№301)</a>.
- Сетевой трафик:

| Действие пользователя              | Средний трафик на одно действие | Нагрузка на сеть (Мбит/с) |
| ---------------------------------- | ------------------------------- | ------------------------- |
| Регистрация и Авторизация          | 200 КБ                          | 6.56                      |
| Создание трансляции                | 200 КБ                          | 1.56                      |
| Отправка сообщения в чат           | 450 Б                           | 0.21                      |
| Обновление страницы (рекомендации) | 2 МБ                            | 1328                      |
| Просмотр своего канала             | 2.5 МБ                          | 85.53                     |
| Редактирование своего канала       | 2 КБ                            | 0.0066                    |
| Просмотр онлайн трансляции         | 3 МБ                            | 2401                      |
| Отслеживание канала                | 1 КБ                            | 0.07                      |
| Оплата подписки                    | 1 КБ                            | 0.0065                    |
| Удаление сообщений                 | 300 Б                           | 0.005                     |
| Блокировка других пользователей    | 400 Б                           | 0.0013                    |
| Создание клипа                     | 2 МБ                            | 80.0                      |
| Сохранение трансляции              | 10 МБ                           | 100                       |
| Просмотр видеоплеера               | 1 МБ                            | 332                       |
| Пожаловаться на трансляцию         | 500 Б                           | 0.00008                   |

**Нагрузка на сеть** была расчитана по формуле: Нагр. на сеть = (ср. трафик на действие) \* RPS \* 8/ 1 048 576.

Средний сетевой трафик на одного пользователя: 0.104 Гбит/день.
Суммарная нагрузка на сеть: ~4335 Мбит/с. Тогда пиковая нагрузка 2 \* 4335 = 8700 Мбит/с.
