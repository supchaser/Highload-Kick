...
- Алгоритм для L7 балансировки - Round Robin, т.к. Least connection хорошо для блокирующего софта, я буду использовать асинхронные подход в приложениях.
...
- С помощью DNS направляем весь входящий трафик в один дата-центр (чтобы не «распылять» трафик по миру).
