...
- Новые сообщения на время фолбака не отсылаем в WebSocket-клиенты, а пишем в Kafka-топик — с мониторингом длины очереди и автобарьером (если очередь > X, то новые сообщения полностью дропаются).
...
- Метрики и алерты: процент запросов, ушедших на каждый уровень фолбака; время жизни кэша.
...
- SLI/SLO: отслеживать долю запросов в fallback и настраивать алерты, если она > x%.
...
- Мониторинг фоновых задач: алерты на необработанные проверки и превышение очередей.
