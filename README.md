...
| Сообщения пользователя(текст)           | 100 Б              | 5                   | 250 млн          | 23,28           |
...
- S3: используем для хранения больших файлов.

## 7. Алгоритмы

### 1. Алгоритм прямой трансляции

Область применения: стриминговые сервисы, где важны качество передаваемой картинки, звука и скорость предачи.

Описание алгоритма:

1. Захват потока:

- Стример отправляет видео в "сервер захвата" по RTMP.
  Почему именно RTMP, а не WebRTC: RTMP всё-таки имеет чуть большую задержку, чем WebRTC, но она не критична. К тому же RTMP предоставляет уже настроенный сервер для перекодирования HLS/DASH. Ещё важным плюсом будет возможность простого масштабирования. WebRTC всё же используется в случаях, где нужна быстрая реакция, например в какой-нибудь онлайн игре.

2. Кодирование:

- Сервер в реальном времени делает несколько версий потока в разных качествах.

3. Хранение сегментов и отдачи через CDN:

- Видео разбивается на короткие сегменты, которые CDN раздаёт по всему миру с адаптивным битрейтом (плеер у зрителя сам выбирает качество трансляции в зависимости от скорости интернета).

### 2. Алгоритм RealTime чата

Область применения: сервисы, где необходимо обеспечение стабильной работы чата при больших нагрузках.

Описание алгоритма:

1. Подключение клиентов к серверу чата:

- Каждый пользователь устанавливает WebSocket-соединение, по которому отправляет и получает сообщения в реальном времени.

2. Обработка входящих сообщений:

- Сервер чата принимает сообщение, сразу же ретранслирует всем, кто находится на этой трансляции.
- Параллельно сохраняем сообщение в Cassandra.

3. История чата:

- Будет восстанавливаться из Cassandra (берем последние 1000 записей для данной трансляции).

### 3. Алгоритм модерации RealTime чата

Область применения: сервисы с какой-либо цензурой, сервисы с большим количеством пользователей.

Описание алгоритма:

1. Первичная проверка:

- Составляется список запрещенных слов, если сообщение содержит в себе что-то запрещенное, то блокируем его.

2. Вторичная проверка:

- Используем ML-классификатор (лёгкая модель на базе BERT, многоязычная), который разделит сообщения по определенным классам в зависмости от уверенности.
  Допустим бот выставил для данного сообщения тэг "токсичность" с уровнем уверенности "высокая". Тогда в зависимости от правил чата выдаем данному пользователю бан или мут. Если уверенность "низкая", то отправляем на ручную проверку модераторам.

3. Обновление модели:

- Те случаи, где вмешался ручной модератор, мы добавим в обучение модели.

### 4. Рекомендации

Область применения: сервисы, которые используют какие-либо рекомендации на основе статистики.

1. Сбор данных: взаимодействия пользователей с контентом отправляются ввиде событий в Apache Kafka -> далее из потока событий данные попадают в PostgreSQL и Cassandra (ETL процессы).

- Поступление событий:
- Читаем данные из PostgreSQL (user_activity, user_preferences, paid_subscription) и Cassandra (follow). Объединяем данные во временные таблицы. Считаем общее количество взаимодействий пользователя с определённым контентом, суммарное время просмотров, факты подписки, факты отправки сообщений на стриме.

2. Фильтрация и агрегирование:

- Агрегатные метрики:
  - Число просмотров
  - Сумарное время, время проведенное на данном канале
  - Факт платной или бесплатной подписки
  - Активность в чате
- На основе этих метрик можно сформировать матрицу взаимодействий где строками будут пользователи, а каналы - столбцами. Для каждой ячейки такой матрицы будут вычисляться interaction*score's.
  `interaction_score` будем вычислять по формуле: interaction_score = a * watch_time + b \* messages_count + c \* follow_flag + d \* paid_sub_flag, где a, b, c и d - весовые коэффициенты.
  Также должна быть еще добавка в зависимости от препочтений пользователя по категориям или тэгам (user_preferences).
  - Фильтрация мусора:
    Исключаем каналы/стримы, которые неактивны (из channel_statistics).
  - После всех расчетов получаем список рекомендаций для пользователей.
  - Для того, чтобы не показывать одно и тоже
  - Для того, чтобы повысить качество рекомендаций будем проводить A/B-тесты разных наборов весовых коэффициентов.
  - Для поддержания актуальности рекомендаций введем временное затухание: более свежие взаимодействия весят больше, а старые - меньше.
  - Также нужно учитывать релевантность канала.
- Записываем полученные рекомендации в таблицу recommendation (только топ-N результатов для данного пользователя).

3. Получение рекомендаций пользователем

- При заходе на платформу пользователь быстро получает необходимые рекомендации из-за низкой задержки на доступ Tarantool.

## 8. Технологии

### СУБД и другие технологии

| Технология    | Область применения                   | Мотивация                                                                                                                                   |
| ------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| PostgreSQL    | Хранение данных                      | Надежная реляционная база с поддержкой ACID и сложных SQL-запросов, оптимальна для структурированных данных                                 |
| ClickHouse    | Хранение данных                      | Высокопроизводительная колоночная СУБД для аналитических запросов, позволяющая обрабатывать большие объемы данных с минимальными задержками |
| Cassandra     | Хранение данных                      | Горизонтально масштабируемая NoSQL база, обеспечивающая высокую доступность и устойчивость в распределенной архитектуре                     |
| Redis         | Хранение данных, сессий пользователя | Быстрая in-memory база для кэширования и хранения сессий, обеспечивает минимальные задержки доступа                                         |
| Kafka         | Потоковая передача сообщений         | Масштабируемый брокер сообщений, обеспечивающий надежную и быструю передачу данных между микросервисами в реальном времени                  |
| ElasticSearch | Поиск                                | Мощный движок для полнотекстового поиска, позволяющий быстро индексировать и находить данные даже в больших объемах информации              |
| S3            | Хранение данных                      | Облачное объектное хранилище, обеспечивающее высокую доступность, масштабируемость и экономичное хранение больших объемов данных            |
| Tarantool     | Хранение данных                      | Быстрая in-memory база с поддержкой Lua-скриптов, оптимизированная для операций с низкой задержкой                                          |
| CDN           | Быстрая раздача статики              | Снижает задержки и нагрузку на центральные серверы за счет доставки кэшированного контента из геораспределенных узлов                       |
| Envoy         | Балансировка нагрузки, прокси-сервер | Гибкий L7-прокси для динамической маршрутизации трафика и балансировки нагрузки, что улучшает отказоустойчивость микросервисной архитектуры |
| WebSocket     | Обмен данными в реальном времени     | Обеспечивает постоянное двустороннее соединение между клиентом и сервером, оптимально для чатов, уведомлений и онлайн-игр                   |
| RTMP          | Потоковая передача видео и аудио     | Протокол для потоковой передачи мультимедийных данных, широко используемый для прямых трансляций с минимальной задержкой                    |

### Языки программирования

| Технология | Область применения                             | Мотивация                                                                                                                              |
| ---------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Go         | Backend                                        | Высокая производительность, простота параллелизма и статическая типизация                                                              |
| Python     | Вычисления и обработка данных ML               | Обширная экосистема библиотек для анализа данных и машинного обучения, упрощает прототипирование и разработку аналитических приложений |
| TypeScript | Frontend                                       | Улучшенная разработка на основе JavaScript с поддержкой статической типизации, повышает надежность и масштабируемость проектов         |
| Lua        | Скрипты для Tarantool                          | Лёгкий и быстрый язык скриптов, оптимально интегрируется с Tarantool для настройки бизнес-логики в реальном времени                    |
| Kotlin     | Мобильное приложение (Android)                 | Современный, безопасный и лаконичный язык для разработки Android-приложений с полной совместимостью с Java                             |
| Swift      | Мобильное приложение (iOS)                     | Нативный язык для iOS, обеспечивающий высокую производительность и тесную интеграцию с экосистемой Apple                               |
| Vue        | Frontend                                       | Легкий и гибкий фреймворк для создания динамичных и интерактивных пользовательских интерфейсов с низким порогом входа                  |
| gRPC       | Вызов методов удалённых серверов               | Обеспечивает высокопроизводительный RPC с использованием HTTP/2 и Protocol Buffers, минимизируя задержки в распределенных системах     |
| Protobuf   | Протокол сериализации структурированных данных | Эффективная и компактная сериализация данных, упрощающая обмен информацией между микросервисами                                        |

### DevOps

| Технология | Область применения                        | Мотивация                                                                                                                                    |
| ---------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Kubernetes | Управление контейнерами                   | Обеспечивает автоматизацию развертывания, масштабирование и управление микросервисной архитектурой, повышая отказоустойчивость               |
| Docker     | Контейнеризация приложений и их окружения | Позволяет создавать изолированные, повторяемые и портативные контейнеры, упрощая разработку, тестирование и развертывание приложений         |
| GitLab CI  | CI/CD                                     | Интегрированное решение для непрерывной интеграции и развертывания, тесно связанное с системой контроля версий, что упрощает DevOps процессы |
| Helm       | Управление пакетами для Kubernetes        | Упрощает развертывание приложений в Kubernetes с помощью шаблонов и версионирования, ускоряя процессы обновлений и масштабирования           |
| Ansible    | Конфигурационное управление               | Обеспечивает простое и эффективное управление конфигурацией серверов, автоматизируя рутинные задачи и обновления                             |

### Модель для ML

| Технология | Область применения | Мотивация                                                                                                                                          |
| ---------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| BERT       | NLP                | Обеспечивает глубокое понимание контекста, улучшая качество задач модерации, поиска и анализа текстов за счет предобученных языковых представлений |

### Системы обеспечения качества продукта

| Технология | Область применения                | Мотивация                                                                                                                                          |
| ---------- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Prometheus | Сбор и хранение метрик            | Эффективно собирает метрики в реальном времени, позволяя мониторить производительность и состояние приложений с минимальной задержкой              |
| Grafana    | Дашборды, визуализация метрик     | Предоставляет гибкие возможности для визуализации метрик, позволяя создавать настраиваемые дашборды для быстрого анализа и выявления аномалий      |
| Jaeger     | Трассировка распределённых систем | Позволяет отслеживать цепочки вызовов между микросервисами, выявлять узкие места и оптимизировать производительность за счёт подробной трассировки |

## 9. Обеспечение надёжности

Для построения отказоустойчивой архитектуры будем использовать несколько методов:

1. Резервирование:

- Резервирование ресурсов: выделяем узлу больше CPU/RAM, чем он обычно потребляет, чтобы при резком росте нагрузки или запуске «тяжёлой» задачи он не «упал».
- Резервирование физических компонетов (сервера, диски и т.д.): используем RAID-массивы, дублируем сетевые карты, блоки питания и материнские платы. Критически важные сервера дублируем.
- Резервирование ДЦ: нагрузка распределяется между несколькими ДЦ для повышения общей отказоустойчивости и доступности системы, что позволяет сохранять работу даже в случае выхода из строя одного из центров [см. пункт 4](#3-глобальная-балансировка-нагрузки)</a>.
- Резервирование БД: реплецирование [см. пункт 6](#6-физическая-схема-бд).
- Резервирование логики: добавляем дополнительный бэкенд с облегчённой логикой, который кэширует данные, с которыми работает продакшен-сервис, и в случае его отказа отдаёт их из собственного кэша.

2. Сегментирование:
   Разбиваем нашу систему на API-кластеры, чтобы сбой или перегрузка в одном сегменте не сказалась на остальных.
...
Группируем кластеры API в группы:
...
- Самые используемые функции должны работать всегда
- Проблема не должна выходить за пределы группы
- Тяжелые, нестабильные страницы не должны класть соседей
- Сложные функции стараемся выделять в отдельные сервисы
...
- Мониторинг:
  Используем Prometheus и Grafana (или что-то похоже) для отслеживания состояния системы.

- Для сервера:
  Использем VRRP. Суть VRRP заключается в том, что несколько маршрутизаторов объединяются в виртуальную группу с общим IP-адресом.

- Контейнеризация и оркестрация:
  Автоматизации отказоустойчивости приложений через replica sets и auto-scaling в Kubernetes.
  ReplicaSet (RS) — это объект Kubernetes, который гарантирует, что в кластере всегда будет работать заданное количество идентичных подов (реплик) приложения. Если под падает, ReplicaSet автоматически создаёт новый.
  Auto-Scaling — это автоматическое увеличение/уменьшение количества подов.

4. Graceful Shutdown:

- Получение сигнала остановки: перехватываем системные сигналы (SIGTERM, SIGINT) для корректной и безболезненной остановки приложения.
- Отказ от новых запросов: у веб-серверов закрываем listening-порт, но продолжаем обработку активных соединений; у Kafka отписываемся от топика.
- Завершение текущих операций: даем X секунд на завершение (например, 30 сек для HTTP-запросов). Если таймаут истёк — логировать ошибку и завершать процесс.

Паттерн для Graceful Shutdown:

- Circuit Breaker - зависшие shutdown-процессы не блокируют всю систему. Например, можно использовать Hystrix или Resilience4j.

5. Graceful Degradation:

Сценарии отработки Graceful Degradation:

1. Отказ chat service:
   Сервиса chat'а будет лежать, но стрим должен продолжаться.

- Показываем N последних сообщений из кэша (Redis) с настроенным TTL (n мин).
- Новые сообщения на время фолбака не отсылаем в WebSocket-клиенты, а пишем в Kafka — с мониторингом длины очереди и автобарьером (если очередь > X, то новые сообщения полностью дропаются).
- Статичное уведомление в UI: «Чат временно недоступен».
- Перенаправить зрителей на внешние сервисы для общения.

2. Сбой Recommendation Service:
   Сервис рекомендаций не отвечает или даёт таймауты.

- Ставим некоторый timeout в API-gateway и при первом признаке деградации сразу отдаем fallback.
- Первый уровень фолбака: попытка загрузить похожие стримы по категории (cache miss -> lookup в Redis).
- Второй уровень: выдача топ-10 самых популярных стримов за последние 24 ч (кэшируется на edge-нодах, обновление раз в час).
- Третий уровень: хардкод-список официальных каналов.

3. Отказ Search Service:
   Поисковый микросервис лежит под нагрузкой, не успевает обрабатывать запросы.

- Ставим некоторый timeout в API-gateway и при первом признаке деградации сразу отдаем fallback.
- Fallback — prefix-match по локальному фронтенд-кэшу (history/suggestions).
  -Ограничение выдачи первыми 5–10 совпадениями.
  -При длинных или «сложных» запросах (опечатки, спецсимволы) показывать: «Поиск временно недоступен или не дал результатов, попробуйте позже».
- Динамическое расширение кэша: при каждом успешном поиске на бекенде обновлять локальные подсказки.

4. Проблемы со Storage Service (CDN/объектным хранилищем):
   S3 хранилище или CDN недоступно/медленно отдаёт сегменты.

- Переключиться на резервный CDN/реплику в другом регионе.
- Отключить не-критичные сегменты (например, рекламу).
- Буферизовать и кэшировать уже загруженные куски.

5. Сбой auth service:
   Модуль авторизации временно недоступен.

- Trust but verify later для активных сессий:
  -Разрешаем продолжить стрим, используя токен из кэша.
  -TTL «доверенного» токена не более n мин.
- Новые пользователи: вместо формы регистрации показываем «Регистрация временно недоступна, попробуйте через n мин» + предлагать OAuth-вход (Google/Twitter) как альтернативу.

4. Асинхронные паттерны:

Применение асинхронных паттернов, таких как CQRS и Outbox, позволяет чётко отделить чтение от записи, синхронизировать изменения данных и повысить надёжность работы системы. А паттерн Saga обеспечивает согласованность информации между несколькими базами данных без необходимости использовать тяжёлые распределённые транзакции.

## 10. Схема проекта

![Highload drawio](https://github.com/user-attachments/assets/1b12afef-8e6f-49c8-b0e1-1ca0ae8a64c6)

# 11. Список серверов

В [пункте 3](#3-глобальная-балансировка-нагрузки) было определено, что пиковый RPS, который может приходиться на один сервер 52,5.
Всего 12 ЦОДов, предполагается, что нагрузка между ними будет распределена равномерно, тогда логичным будет выбрать одинаковые сервера под эти ЦОДы.

Модель развертывания: Kubernetes.

Для подсчета распределения ресурсов между сервисами воспользуемся [пунктом 2](#2-расчет-нагрузки).

## Распределение ресурсов по сервисам

| Сервис         | Целевая пиковая нагрузка приложения | CPU | RAM (GB) | Net (Gbit/s) |
| -------------- | ----------------------------------- | --- | -------- | ------------ |
| auth           | 8.4                                 | 1   | 1        | 0.0131       |
| user           | 10                                  | 1   | 1        | 0.1641       |
| stream         | 285.4                               | 30  | 300      | 2.7680       |
| channel        | 25.9                                | 3   | 3        | 0.1642       |
| content        | 22                                  | 3   | 3        | 0.3438       |
| recommendation | 166.6                               | 17  | 17       | 2.6031       |
| search         | 40                                  | 4   | 4        | 0.3125       |
| analytics      | 83.3                                | 9   | 9        | 0.0853       |
| notification   | 4.2                                 | 1   | 1        | 0.0003       |
| billing        | 1.66                                | 1   | 1        | 0.0001       |
| storage        | 83.3                                | 9   | 9        | 0.3250       |

> **Примечание:**
>
> - Удельное потребление ресурсов сервисом на единицу нагрузки:
>   - 1 CPU на 10 RPS
>   - 1 Гб requests и 2 Гб limit на 10 RPS
> - Расчет Net был произведен по следующей формуле: Net = ((ср. трафик на действие ) \* пиковый RPS \* 8)/ 1024
>   - Пример для auth: (0.2 MB\*8.4 RPS\*8)/1024 ~= 0.0131 Gbit/s
